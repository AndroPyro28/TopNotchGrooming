  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await axios.get(`/api/admin/getScheduleToday/${dateToday}`, {
          headers: {
            userinfo: Cookies.get('userToken')
          }
        })
    ...
    <BackdropModal>
      <LiveStreamModalContainer>
        ...
        <ScheduleList>
          ...
          {
            scheduleList.length === 0 ? <h2 style={{color:"gray"}}>No Schedule today...</h2> :
            scheduleList.map(schedule => (
              <Schedule key={schedule.appointment.id} data={schedule} />
            ))
          }
        ...
        <PaginationContainer>
        <i class="fa-solid fa-chevron-left left"></i> 1 / 2 <i class="fa-solid fa-chevron-right right"></i>
        </PaginationContainer>

        <ButtonContainer>
            <button className="cancelBtn" onClick={() => setToggleModal(false)}>Cancel</button>
